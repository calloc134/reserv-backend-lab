...
export async function findReservationByDateRange(
    ...
    SELECT
      ...
      rord.date,
      rord.slot,
      CASE WHEN rord.status = 'reserved' THEN res.user_id ELSE NULL END AS user_id
    FROM reservation_or_disabled rord
    JOIN room ON rord.room_uuid = room.room_uuid
    LEFT JOIN reservation res ON rord.reservation_uuid = res.reservation_uuid
    WHERE rord.date >= ${start_date} AND rord.date <= ${end_date}
    ORDER BY rord.date, rord.slot;
  `;
